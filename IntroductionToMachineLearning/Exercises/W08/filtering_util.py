...
def plot_x_and_y_combined(data_x,data_y, scale = 5):
    last_key = list(data_x.keys())[-1]
    plt.figure(figsize=(12, 6))
    for i in list(data_x.keys()):
        x = np.linspace(0, len(data_x[i]), len(data_x[i]))
    ...
    plt.subplot(2, 1, 1)
    plt.title('Combined Pupil Coordinate Over Frames', fontsize=12)
    plt.xlabel('# Frame', fontsize=10)
    plt.ylabel('Pupil Coordinate (x,y)', fontsize=10)
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend()


    plt.subplot(2, 1, 2)
    plt.title('Derivate of Combined Pupil coordinate', fontsize=12)
    plt.xlabel('# Frame', fontsize=10)
    plt.ylabel('Pupil Coordinate (x,y)', fontsize=10)
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend()
...
def plot_fixation_eval(raw_csv, intervals, means_csv, annotate_errors=True, one_based=False):
    ...
    means_csv: CSV with mean (or median) pupil centers; rows = fixations. Needs columns: px, py
    one_based: if True, label means as 1..N instead of 0..N-1
    """
    ...
    preds = np.full(len(df), 'saccade', dtype=object)
    for k, (s, e) in intervals.items():
        preds[s:e+1] = str(int(k))
    df['pred_label'] = preds

    is_fix = df['pred_label'] != 'saccade'
    is_sacc = ~is_fix
    df.loc[is_fix, 'pred_fix'] = df.loc[is_fix, 'pred_label'].astype(int)
    df['correct'] = False
    df.loc[is_fix, 'correct'] = (df.loc[is_fix, 'pred_fix'] == df.loc[is_fix, 'true_fix'])

    # --- plot samples ---
    plt.figure(figsize=(7,7))
    plt.scatter(df.loc[is_sacc,'px'], df.loc[is_sacc,'py'],
            color=tab10[1], s=10, label='Saccade')

    plt.scatter(df.loc[is_fix & df['correct'],'px'], df.loc[is_fix & df['correct'],'py'],
                color=tab10[0], s=12, label='Fixation (correct)')

    mis = df[is_fix & ~df['correct']]
    plt.scatter(mis['px'], mis['py'],
                color='red', s=20, label='Fixation (wrong)')


    if annotate_errors and not mis.empty:
        for _, row in mis.iterrows():
            plt.annotate(f"({int(row['pred_fix'])}/{int(row['true_fix'])})",
                         (row['px'], row['py']),
                         textcoords="offset points", xytext=(3,3),
                         fontsize=8, color=tab10[3])

    # --- overlay fixation means; label by row index ---
    means = pd.read_csv(means_csv) 
    plt.scatter(means['px'], means['py'], color='black', s=60, marker='x', label='Fixation means')

    for i, r in means.reset_index(drop=True).iterrows():
        label = i + 1 if one_based else i
        plt.annotate(str(label), (r['px'], r['py']),
                     textcoords="offset points", xytext=(5,5),
                     fontsize=10, color='black')

    plt.xlabel("px")
    plt.ylabel("py")
    plt.title("Fixation interval evaluation")
    plt.legend()
    plt.gca().invert_yaxis() 
    plt.tight_layout()
    plt.show()
